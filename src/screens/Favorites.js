...
export default function Favorites({ navigation, route }) {
	...
	const renderItem = ({ item, index }) => (
		<Edition item={item} index={index} projectDomain={item.projectDomain} navigation={navigation} route={route} onChangeDownloaded={onChangeEdition} onChangeFavorite={onChangeEdition}/>
	);
		...
		<View style={[themeBackgroundColor, styles.editionContainer]}>
			...
			{isLoading ? <ActivityIndicator style={styles.mt5} /> : (
				<FlatList
					...
					ListEmptyComponent={<Text style={[themeTextColor, styles.ml2]}>No favorites were found...</Text>}
					data={editions}
